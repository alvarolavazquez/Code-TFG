...
const listByShop = (req, res) => {
  Order.find({ "products.shop": req.shopId })
...
const findProductOrder = async (req, id, name)  => {
  ...
  .exec((err, order) => {
    ...
    for (var i=0;i< order.products.length;i++){
      if (order.products[i].product.File !== undefined && order.products[i].product.File==name) {return order.products[i].product.TOKEN;};
    }    
  });
};
